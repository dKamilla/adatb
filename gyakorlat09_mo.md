...
with p.name as Nev, count(m.title) as Db // segéd lekérdezés
where Db  > 1 // having 
return Nev, Db
```
...
```js
match (p:Person)-[:ACTED_IN]->(m:Movie)<-[:DIRECTED]-(p:Person) // de lehet úgy is, hogy egymás alatt több MATCH utasítás; illeve vhogy WHERE-el is lehet csinálni
return p.name
```
...
```js
Create index i_person_born //indexelni csúcsokra vagy élekre lehet
for (p:Person)
on(p.name, p.born) // összetett index, kell a zárójel

show indexes // ezzel lehet utána külön utasításban megnézni, hogy milyen indexek vannak 
```
...
```js
Profile // ezzel látjuk, hogy indexet használ, azt amit csináltunk az ekőzőben, a plan fülnél lehet látni, ilyen kis adatoknál mindegy amúgy, de sok adatnál a futási időt rövidíti
match (p:Person)
